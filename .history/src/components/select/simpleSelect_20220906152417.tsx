...
export const LabelWithColor = ({ label, color }: { label: string; color: string }) => <span style={{ background: color }}>{label}</span>;
  ...
  return (
    <label ref={rootEl} className="simpleSelect">
      <div ref={inputRef} onMouseDown={() => setOpen(true)} className={`input d-flex ${open ? "active" : ""}`}>
        <div className="flex-grow-1 content">{value ? <LabelWithColor label={value.label} color={value.color} /> : placeholder}</div>
