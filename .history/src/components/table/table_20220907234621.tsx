...
// set className expanded for tr with DOM manipulation
const onExpand = (row: any, isExpand: boolean, rowIndex: number, e: SyntheticEvent) => {
  let rowElement;
  //@ts-ignore
  ...
  // reset classNames expanded
  for (let i = 0; i < rowElement.parentElement.children.length; i++) {
    rowElement.parentElement.children[i].classList.remove("expanded");
  }
...
export const Table = ({ data, Document }: { data: CargoDocumentType[]; Document: any }) => {
  return (
    <BootstrapTable
      selectRow={{
        mode: "checkbox",
