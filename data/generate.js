...
function writeData() {
  ...
  for (let i = 0; i < 300; i++) {
    ...
    data.cargo.push({
      ...
      portLocation: "3 скл причал",
      actFile: Math.round(Math.random()) ? { name: "Накладная_для_Умки (1).doc", date: "21.10.2021", label: "Заказ №4329" } : undefined,
      consignmentFile: Math.round(Math.random()) ? { name: "Накладная_для_Умки (1).doc", date: "21.10.2021", label: "Заказ №4329" } : undefined,

      // not in scheme
      port: ports[rand(ports.length + 4)],
    });
  }
  fs.writeFileSync("data.json", JSON.stringify(data, 2, 2));
}
writeData();
